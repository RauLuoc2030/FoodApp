...
class Recipe_NutritionService {
  ...
  Recipe_Nutrition getRecipe_NutritionByRecipeId(int id) {
    // Find the Recipe_Nutrition with the given ID
    return Recipe_Nutritions.firstWhere((element) => element.id == id);
  }
  List<Recipe_Nutrition> getNutritionByRecipeID(int? rid) {
    // Find the Recipes with the given category
    return Recipe_Nutritions.where((element) => element.recipeID == rid).toList();
  }
  double? getValue(int? rid,int? nutriid) {
    // Find the Recipes with the given category
    return Recipe_Nutritions.where((element) => element.recipeID == rid && element.nutritionID == nutriid).first.value;
  }
